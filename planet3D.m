...
function planet_surface = planet3D(planet,opts)
    ...
    dir = [fileparts(which('planet3D.m')), '/'];
    ...
    if ~strcmpi(planet,'Earth Coastlines')
        ...
        if strcmpi(planet,'Earth Cloudy')
            cdata = imread([dir, 'resources/earth.png'])+imread([dir, 'resources/clouds.png']);
        elseif strcmpi(planet,'Earth Night Cloudy')
            cdata = imread([dir, 'resources/earthnight.png'])+0.1*imread([dir, 'resources/clouds.png']);
        else
            cdata = imread(strcat(dir, 'resources/', lower(planet), '.png'));
        end
    ...
    if strcmpi(planet,'Earth Coastlines')
        ...
        coastlines_data = struct2array(load([dir, 'resources/coastlines_data']));
    ...
    if strcmpi(planet,'Saturn')
        ...
        % reads in image
        cdata_rings = imread([dir, 'resources/saturnrings.png']);
